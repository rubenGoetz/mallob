#!/bin/bash
#SBATCH --nodes=$DS_NODES # 1 for now?
...
for i in $(seq $DS_FIRSTJOBIDX $DS_LASTJOBIDX | shuf) ; do
    ...
    cmd="$build/mallob -mono-app=SATWITHPRE -pb=1 -pjp=999999 -pef=2 -terminate-abruptly=1 -mono=$f -jwl=$timeout -T=$(($timeout+30)) -wam=60``000 -pre-cleanup=1 \
    ...
    -rpa=1 -pph=${SLURM_NTASKS_PER_NODE} -mlpt=50``000``000 -t=${SLURM_CPUS_PER_TASK} \
    -satsolver=$solver_str -isp=0 -div-phases=1 -div-noise=0 -div-seeds=1 -div-elim=0 -div-native=0 -scsd=0 \
